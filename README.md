...
- **공유는 링크 하나로 단순화**: 처음엔 Web Share API에 문구·이미지까지 같이 실어 보냈는데, 일부 앱(카카오톡 등)이 이를 사진 메시지와 텍스트 메시지로 쪼개서 보내는 걸 확인했습니다. 지금은 URL 하나만 공유하고, 카드 이미지는 `og:image` 메타데이터를 통해 받는 앱이 링크 미리보기로 알아서 그려주도록 역할을 분리했습니다.
