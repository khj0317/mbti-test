...
- **네이티브 공유 시트 연동** — 모바일에서 "결과 공유하기"를 누르면 Web Share API로 OS 공유 시트가 뜨고, 카카오톡·인스타그램 등 설치된 앱에 유형 카드 이미지 + 링크가 바로 공유됩니다. 지원하지 않는 환경(주로 데스크톱)에서는 클립보드 복사로 자동 대체됩니다.
...
- **공유 이미지 캐싱**: 16가지 유형 + 기본 카드, 즉 존재할 수 있는 OG 이미지는 17개뿐입니다. 그래서 `/api/og` 응답에 `Cache-Control: immutable`을 걸어서 매번 폰트를 새로 받아오지 않고 CDN 캐시로 즉시 응답하도록 했습니다. 공유 버튼도 첫 요청(콜드 캐시)의 지연을 감안해서 "공유 준비 중..." 로딩 상태를 넣었습니다.
